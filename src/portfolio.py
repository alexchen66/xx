...
import pandas as pd
import numpy as np
from config import LONG_ONLY_N, TOP_PCT


def build_long_only(predictions: pd.DataFrame, score_col: str, top_n: int = LONG_ONLY_N) -> pd.DataFrame:
    """Equal-weight top-N stocks by score on each rebalance date."""
    portfolios = []
    for date, df in predictions.groupby("date"):
        df = df.nlargest(top_n, score_col).copy()
...
def build_long_short(
    ...
    """Equal-weight top decile long, bottom decile short."""
    portfolios = []
    for date, df in predictions.groupby("date"):
        ...
        long_df["weight"]  =  1.0 / n
        short_df["weight"] = -1.0 / n
        long_df["side"]    = "long"
        short_df["side"]   = "short"
