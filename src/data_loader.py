...
EXTERNAL_DOWNLOADS = Path(r"C:\Users\johnh\Finance\finance project\ML")


def _download_file(filename: str) -> Path:
    """Find a raw CSV either in data/raw/downloads or in the parent ML folder."""
    candidates = [
        DOWNLOADS / filename,
        EXTERNAL_DOWNLOADS / filename,
    ]
...
def _to_datetime_safe(series: pd.Series, format: str = None) -> pd.Series:
    ...
    def _parse_one(v):
        v = str(v).strip()
        if v in {"", "nan", "NaN", "NaT", "None"}:
            return pd.NaT
        if v.endswith(".0"):
            v = v[:-2]
        if len(v) == 8 and v.isdigit():
            try:
                return pd.Timestamp(year=int(v[:4]), month=int(v[4:6]), day=int(v[6:8]))
            except ValueError:
                return pd.NaT
        if len(v) == 10:
            try:
    ...
    return pd.Series(pd.array([_parse_one(v) for v in s]), index=series.index)
...
def load_crsp_delist() -> pd.DataFrame:
    ...
    """
    f = _download_file("crsp_delist.csv")
    if not f.exists():
        raise FileNotFoundError(f"Missing {f}")
...
def load_crsp_names() -> pd.DataFrame:
    ...
    """
    f = _download_file("crsp_names.csv")
    if not f.exists():
        raise FileNotFoundError(f"Missing {f}")
...
def load_compustat_quarterly() -> pd.DataFrame:
    ...
    """
    f = _download_file("compustat_fundq.csv")
    if not f.exists():
        raise FileNotFoundError(f"Missing {f}")
...
def load_crsp_compustat_link() -> pd.DataFrame:
    ...
    """
    f = _download_file("crsp_compustat_link.csv")
    if not f.exists():
        raise FileNotFoundError(f"Missing {f}")
...
def load_fama_french() -> pd.DataFrame:
    ...
    """
    f = _download_file("ff_factors_daily.csv")
    if not f.exists():
        raise FileNotFoundError(f"Missing {f}")
...
def check_downloads() -> bool:
    files = {
        "crsp_dsf.csv":             _download_file("crsp_dsf.csv"),
        "crsp_delist.csv":          _download_file("crsp_delist.csv"),
        "crsp_names.csv":           _download_file("crsp_names.csv"),
        "compustat_fundq.csv":      _download_file("compustat_fundq.csv"),
        "crsp_compustat_link.csv":  _download_file("crsp_compustat_link.csv"),
        "ff_factors_daily.csv":     _download_file("ff_factors_daily.csv"),
    }
    print("\n-- Download status --------------------------------")
    all_ok = True
    for name, path in files.items():
        if path.exists():
            ...
            print(f"  OK      {name:<35} {size:>7.1f} MB  [{path.parent}]")
        else:
