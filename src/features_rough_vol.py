...
def compute_all_rough_features(
    ...
    # Rolling 60d beta → idio_ret (pandas rolling cov/var, C-speed)
    def _rolling_beta_idio(g):
        cov  = g["ret"].rolling(60, min_periods=30).cov(g["mktrf"])
        var  = g["mktrf"].rolling(60, min_periods=30).var().clip(lower=1e-10)
        beta = cov / var
        idio = g["ret"] - beta * g["mktrf"]
        g = g.copy()
        g["idio_ret"] = idio
        return g

    crsp = crsp.groupby("permno", group_keys=False).apply(_rolling_beta_idio)
